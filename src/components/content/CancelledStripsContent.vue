...
const flightStrips = computed(() => {
  ...
  return [...stripsStore.cancelled].sort((a, b) => b.last_modified - a.last_modified);
});

const handleStripDrop = (dragData: DragData) => {
  if (dragData.sourceArray === 'Cancelled') {
    return;
  }
...
<template>
  <DropZoneWrapper target-array="Cancelled" :on-drop="handleStripDrop" :accepts="['strip']">
    <vstack gap="1" class="h-full">
      <DraggableWrapper
        v-for="strip in flightStrips"
        :key="strip.id"
        :data="{ type: 'strip', stripId: strip.id, sourceArray: 'Cancelled' }"
      >
        <FlightStrip
          :strip-id="`${panelKey}-strip-${strip.id}`"
          :strip="strip"
          current-array="Cancelled"
        />
      </DraggableWrapper>
    </vstack>
  </DropZoneWrapper>
