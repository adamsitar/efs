...
const handleStripDrop = (dragData: DragData) => {
  if (dragData.sourceArray === 'Planned') {
    return;
  }
...
<template>
  <DropZoneWrapper target-array="Planned" :on-drop="handleStripDrop" :accepts="['strip']">
    <vstack gap="1" class="h-full">
      <DraggableWrapper
        v-for="strip in flightStrips"
        :key="strip.id"
        :data="{ type: 'strip', stripId: strip.id, sourceArray: 'Planned' }"
      >
        <FlightStrip
