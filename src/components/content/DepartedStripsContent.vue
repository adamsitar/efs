...
const flightStrips = computed(() => {
  ...
  return [...stripsStore.departed].sort((a, b) => b.last_modified - a.last_modified);
});

const handleStripDrop = (dragData: DragData) => {
  if (dragData.sourceArray === 'Departed') {
    return;
  }
...
<template>
  <DropZoneWrapper target-array="Departed" :on-drop="handleStripDrop" :accepts="['strip']">
    <vstack gap="1" class="h-full">
      <DraggableWrapper
        v-for="strip in flightStrips"
        :key="strip.id"
        :data="{ type: 'strip', stripId: strip.id, sourceArray: 'Departed' }"
      >
        <FlightStrip
          :strip-id="`${panelKey}-strip-${strip.id}`"
          :strip="strip"
          current-array="Departed"
        />
      </DraggableWrapper>
    </vstack>
  </DropZoneWrapper>
