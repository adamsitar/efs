...
// Compute children for Group component
const groupChildren = computed(() => {
  if (!props.node.children) return [];

  return props.node.children.map(child => ({
    id: child.type === 'splitter' ? `${child.id}-group` : child.id,
    type: determineType(child)
  }));
});

function determineType(node: LayoutNode): FocusableType {
  if (node.type === 'panel') return 'panel';
  if (node.type === 'splitter') return 'group';
...
<template>
  ...
  <Group v-if="node.type === 'splitter' && node.children && node.children.length >= 2" :group-id="`${node.id}-group`"
    :direction="node.direction || 'horizontal'" :children="groupChildren" :enable-ctrl-arrow="true" :enable-tab="true"
    :enable-arrow="true" :enable-num="isRoot">
    <SplitterGroup :direction="node.direction || 'horizontal'" class="h-full w-full">
      <template v-for="(child, index) in node.children" :key="child.id">
        ...
        <SplitterResizeHandle v-if="index < node.children.length - 1" :class="[
  ...
  <splitter v-else-if="node.type === 'splitter' && node.children && node.children.length === 1"
